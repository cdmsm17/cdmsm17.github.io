...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | cdmsm17")[0];
}
...
function createtime2() {
  var ascll2 = [`NCC2-036`, `调用前置摄像头拍照成功，希望你别在干坏事`, `Photo captured: `, ` 🤪 `];
  ...
  setTimeout(console.log.bind(console, "%c WELCOME %c 希望本站对你有帮助", "color:white; background-color:#23c682", ""));

  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by cdmsm17 %c 能V我50吗😋 ",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| cdmsm17") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('cdmsm17的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
